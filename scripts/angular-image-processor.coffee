angular.module('tbImageProcessor', []).service('imageProcessor', ($q, $rootScope) ->
  {
    ...
    _scaleImage: (origImage, config) -> #fLetterBox
      ...
      if sourceDims.fScaleOnWidth
        ...
        sourceDims.height = Math.floor(if config.orientation < 5 then 1 / targetAR * origImage.width else targetAR * origImage.width)
      else
        sourceDims.width = Math.floor(if config.orientation < 5 then targetAR * origImage.height else 1 / targetAR * origImage.height)
